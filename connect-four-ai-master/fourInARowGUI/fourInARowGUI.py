    ...
    while True:
        for event in pygame.event.get():  # event handling loop
            if event.type == QUIT:
                pygame.quit()
                sys.exit()
            elif event.type == MOUSEBUTTONDOWN and not draggingToken and BLUEPILERECT.collidepoint(event.pos):
                # start of dragging on BLUE token pile.
                draggingToken = True
                tokenx, tokeny = event.pos
            elif event.type == MOUSEMOTION and draggingToken:
                # update the position of the BLUE token being dragged
                tokenx, tokeny = event.pos
        ...
        if tokenx != None and tokeny != None:
            drawBoard(board,
                      {'x': tokenx - int(SPACESIZE / 2), 'y': tokeny - int(SPACESIZE / 2), 'color': BLUE})
        else:
            drawBoard(board)
...
def dropHumanToken(board, column):
    animateDroppingToken(board, column, BLUE)
    board[column][getLowestEmptySpace(board, column)] = BLUE
    drawBoard(board)
    pygame.display.update()
